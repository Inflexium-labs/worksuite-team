...
@section('content')
    <div class="row bg-title">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <a href="{{ route('member.team.index') }}" class="btn btn-outline btn-inverse btn-sm pull-left m-r-15">
                                                ...
                                                @foreach ($team->members as $member)
                                                    <tr>
                                                        ...
                                                        <td><a class="btn btn-danger btn-xs unlinkMember"
                                                                data-id="{{ $member->id }}"
                                                                href="{{ route('member.team.remove-member', $team->id) }}"><i
...
@push('footer-script')
    ...
    <script>
        ...
        $('#editTeam').click(function(e) {
            e.preventDefault();
            $.ajaxModal('#teamModal', '{{ route('member.team.edit', $team) }}');
        });

        $('#addMembersForm').submit(function(e) {
            e.preventDefault();
            $.easyAjax({
                url: "{{ route('member.team.add-members', $team) }}",
        ...
        $('#statusBtn').change(function() {
            ...
            $.easyAjax({
                url: "{{ route('member.team.status-update', $team) }}",
                type: "POST",
                data: {
                    'status': status,
                    '_token': '{{ csrf_token() }}'
        ...
        $('#deleteTeam').click(function() {
            swal({
                ...
                if (isConfirm) {
                    $.easyAjax({
                        type: 'DELETE',
                        url: '{{ route('member.team.destroy', $team) }}',
                        data: {
                            '_token': '{{ csrf_token() }}'
                        },
        ...
        $('#editLeader #update').click(function() {
            $.easyAjax({
                url: "{{ route('member.team.update-leader', $team) }}",
                type: "POST",
                data: {
